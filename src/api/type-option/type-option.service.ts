...
@Injectable()
export class TypeOptionService extends MysqlService {
	constructor(
	...
	async many(payload): Promise<any> {
		try {
			const cachedData = await this.cacheService.get(`${process.env.APP_ID}.typeOption.many`, payload);
			...
			await this.cacheService.set(`${process.env.APP_ID}.typeOption.many`, payload, output);
	...
	async one(payload): Promise<any> {
		try {
			const cachedData = await this.cacheService.get(`${process.env.APP_ID}.typeOption.one`, payload);
			...
			await this.cacheService.set(`${process.env.APP_ID}.typeOption.one`, payload, output);
	...
	async drop(payload): Promise<any> {
		...
		try {
			...
			await this.cacheService.clear(`${process.env.APP_ID}.typeOption.one`, payload);
	...
	async create({ user, ...payload }): Promise<any> {
		...
		try {
			...
			const output = await this.typeOptionRepository.save(payload);
	...
	async update({ user, ...payload }): Promise<any> {
		...
		try {
			await queryRunner.startTransaction();
			await this.cacheService.clear(`${process.env.APP_ID}.typeOption.many`);
			await this.cacheService.clear(`${process.env.APP_ID}.typeOption.one`);
