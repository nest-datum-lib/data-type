		...
		try {
			const many = await this.typeOptionService.many({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_DATA_TYPE_TYPE_OPTION_MANY'] ],
					isRequired: true,
				}),
	...
	async one(payload) {
		try {
			const output = await this.typeOptionService.one({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_DATA_TYPE_TYPE_OPTION_ONE'] ],
					isRequired: true,
				}),
				relations: Validators.obj('relations', payload['relations']),
				select: Validators.obj('select', payload['select']),
	...
	async drop(payload) {
		try {
			await this.typeOptionService.drop({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_DATA_TYPE_TYPE_OPTION_DROP'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id'], {
					isRequired: true,
	...
	async dropMany(payload) {
		try {
			await this.typeOptionService.dropMany({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_DATA_TYPE_TYPE_OPTION_DROP_MANY'] ],
					isRequired: true,
				}),
	...
	async create(payload) {
		try {
			const output = await this.typeOptionService.create({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_DATA_TYPE_TYPE_OPTION_CREATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
	...
	async update(payload) {
		try {
			await this.typeOptionService.update({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_DATA_TYPE_TYPE_OPTION_UPDATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
